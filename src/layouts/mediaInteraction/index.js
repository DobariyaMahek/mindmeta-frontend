    ...
    return (
      <ListItem
        ...
        sx={{
          ...
          padding: "10px",
          borderRadius: "5px",
          border: "0.0625rem solid #d2d6da80",
        }}
      >
        <SoftBox
          sx={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100px" }}
        >
          <Box
            sx={{
              display: "flex",
              ...
              width: type !== "audio" ? "100px" : "100%",
              height: type !== "audio" ? "100px" : "100%",
              marginRight: "20px",
            }}
          >
            {/* Render image, video, or audio preview */}
            {type === "image" && (
              <img src={fileUrl} alt={file.name} style={{ width: "100px", maxHeight: "100px" }} />
            )}
            {type === "video" && (
        ...
        {/* Remove file button */}
        <IconButton color="primary" onClick={() => removeMedia(type, index)}>
          <Close />
        </IconButton>
      </ListItem>
    );
  ...
  const renderUploadSection = (type) => (
    <Grid item xs={12} sm={6} md={4}>
      ...
      <List sx={{ marginTop: "20px" }}>
        {media[type].map((file, index) => renderMediaPreview(file, type, index))}
      </List>
    </Grid>
  );
  const clearState = () => {
